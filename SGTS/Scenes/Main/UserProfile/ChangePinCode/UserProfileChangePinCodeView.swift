//
//  UserProfileChangePinCodeView.swift
//  SGTS
//
...
protocol UserProfileChangePinCodeViewOutput: AnyObject,
                                    TextCellWithTitleAtBorderViewModelOutput,
                                    SwitchBiometryCellViewModelOutput,
                                    NextStepButtonViewModelOutput { }

protocol UserProfileChangePinCodeViewLogic: UIView {
    func update(viewModel: UserProfileChangePinCodeModel.ViewModel)
    func displayWaitIndicator(viewModel: UserProfileChangePinCodeFlow.OnWaitIndicator.ViewModel)

    var output: UserProfileChangePinCodeViewOutput? { get set }
}


final class UserProfileChangePinCodeView: UIView, UserProfileChangePinCodeViewLogic, SpinnerDisplayable {
    ...
    private(set) var viewModel: UserProfileChangePinCodeModel.ViewModel?
    ...
    weak var output: UserProfileChangePinCodeViewOutput?
    ...
    private func configure() {
        ...
        tableView.register(cellType: SwitchBiometryCell.self)
...
extension UserProfileChangePinCodeView: UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if let vm = item as? TextCellWithTitleAtBorderViewModel {
            let cell = tableView.dequeueReusableCell(for: indexPath) as TextCellWithTitleAtBorder
            cell.viewModel = vm
            cell.viewModel?.output = output
            return cell
        } else if let vm = item as? SwitchBiometryCellViewModel {
            let cell = tableView.dequeueReusableCell(for: indexPath) as SwitchBiometryCell
            cell.viewModel = vm
            cell.viewModel?.output = output
            return cell
