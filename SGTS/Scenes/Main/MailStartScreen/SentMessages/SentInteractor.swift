...
final class SentInteractor: MailStartScreenBusinessLogic, MailStartScreenDataStore {
    
    // MARK: - Public properties

    var presenter: MailStartScreenPresentationLogic?
    var worker: SentWorkingLogic?

    var idOfSelectedMail = String()
    var someMessagesFromTotal = Int()
    var totalMessages = Int()
    var mailsForDisplay = [EmailMessageWithNeededProperties]()
    var messageTypeFromSideMenu: TabBarManager.MessageType = .sent
    ...
    private func workerFetchMailDataArr() {
        ...
        worker?.getAllStoredEmailsFromFolder(GlobalConstants.sentEmails) { [weak self] result in
            guard let self = self else { return }
