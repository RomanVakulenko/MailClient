...
final class DraftsInteractor: MailStartScreenBusinessLogic, MailStartScreenDataStore {

    // MARK: - Public properties

    var presenter: MailStartScreenPresentationLogic?
    var worker: DraftsWorkingLogic?

    var idOfSelectedMail = String()
    var someMessagesFromTotal = Int()
    var totalMessages = Int()
    var mailsForDisplay = [EmailMessageWithNeededProperties]()
    var messageTypeFromSideMenu: TabBarManager.MessageType = .drafts
    ...
    func updateMailListWithReceiveNewMail(request: MailStartScreenFlow.Update.Request) {
        mailsForDisplay = []
        mailManager.fetchNewMails { [weak self] result in
    ...
    private func workerFetchMailDataArr() {
        ...
        worker?.getAllStoredEmailsFromFolder(GlobalConstants.drafts) { [weak self] result in
            guard let self = self else { return }
            ...
            case .success(let messages):
                ...
                someMessagesFromTotal = 0 //mailsTuple.0
                totalMessages = messages.count //mailsTuple.1
