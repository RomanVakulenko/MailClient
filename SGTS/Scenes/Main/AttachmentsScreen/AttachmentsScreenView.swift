...
protocol AttachmentsScreenViewOutput: AnyObject,
                                      SearchViewOutput,
                                      AttachmentCellViewModelOutput {
    func bottomCellHasBeenDisplayed()
}

protocol AttachmentsScreenViewLogic: UIView {
    func toggleSearchBar(viewModel: SearchViewModel)
    func update(viewModel: AttachmentsScreenModel.ViewModel)
    func displayWaitIndicator(viewModel: AttachmentsScreenFlow.OnWaitIndicator.ViewModel)
    
    var output: AttachmentsScreenViewOutput? { get set }
}


final class AttachmentsScreenView: UIView, AttachmentsScreenViewLogic, SpinnerDisplayable {
    ...
    private(set) var viewModel: AttachmentsScreenModel.ViewModel?
    ...
    weak var output: AttachmentsScreenViewOutput?
    ...
    private func configure() {
        ...
        tableView.register(cellType: AttachmentCell.self)
...
extension AttachmentsScreenView: UITableViewDataSource, UITableViewDelegate {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if let vm = item as? AttachmentCellViewModel {
            let cell = tableView.dequeueReusableCell(for: indexPath) as AttachmentCell
            cell.viewModel = vm
...
//says if the bottom cell has been displayed
